...
stock["Returns"] = stock["Close"].pct_change().fillna(0)
nifty["Returns"] = nifty["Close"].pct_change().fillna(0)
sp500["Returns"] = sp500["Close"].pct_change().fillna(0)

# Ensure Market Data Exists Before Correlation
if nifty.empty or sp500.empty:
    print("⚠️ Warning: Market index data is missing. Correlation cannot be calculated.")
    corr_nifty, corr_sp500 = None, None
else:
...
with pd.ExcelWriter(excel_filename, engine="xlsxwriter") as writer:
    stock.to_excel(writer, sheet_name="Stock Data", float_format="%.4f")
    if not nifty.empty:
        nifty.to_excel(writer, sheet_name="NIFTY 50 Data", float_format="%.4f")
    if not sp500.empty:
        sp500.to_excel(writer, sheet_name="S&P 500 Data", float_format="%.4f")

    # Summary Sheet
    if corr_nifty is not None and corr_sp500 is not None:
